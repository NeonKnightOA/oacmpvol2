...
* **Testing is done with OpenArena ver. 0.8.8**.
    * Optionally, you can download the [gamecode](http://openarena.ws/board/index.php?topic=5296.0) and [(Windows-only) engine](http://openarena.ws/board/index.php?topic=5263.0) from the nightly builds, in order to test the maps with the Possession gametype.
...
These are points which OACMP contributors may or may not follow, but their following is adviced as it ensures a good quality on the overall pack. 

### (For mappers) In general

* **The style of the maps of the pack is up to the mappers themselves.** Remakes and "inspirations" are allowed (after all, OACMP1 has versions of [Facing Worlds](http://unreal.wikia.com/wiki/Recurring_maps%23Face) and [Hyperblast](http://unreal.wikia.com/wiki/Recurring_maps%23Hyperblast)), but the preferences are original maps made specifically for this pack.
...
### (For mappers) On map layout

* **Plan and build your map with optimizations and the game engine's limitations in mind.** Good building techniques are always advisable.
    * From the mapping POV, an unoptimized map brings a higher compiling time and a higher chance of getting BSP-related bugs such as the infamous "Hall of Mirrors", not to mention that it's extremely hard to optimize a map which wasn't built with this in mind.
    * From the playing POV, there's the FPS drop because the engine renders more than it's actually seen and this impacts negatively in the player experience.
...
* **Design your maps with newbs/bots in mind**, and add shortcuts for seasoned players, but avoid as much as possible the obligatory need of knowing trickjumps in order to reach certain items which cannot be accessed normally on foot. As an example, empty small towers which can only be reached by trickjumping are OK, but the same towers holding items is BAD. **This is especially forbidden for key game objectives (flags, skull generators, skull receptacles, etc)** since every kind of player is supposed to have access at them; failure at complying with this generates an unnecessary imbalance. Remember that even the original Q3 was enjoyable without the need of knowing trickjumping.
* Regarding gametype-specific map layout:
    * Regarding the size of your map and the multi-gametype-support nature of OA maps, it may be a good idea to **consider support for other gametypes**. Here's a hint checklist:
        ...
        * Small and medium-sized FFA maps can support **Possession**.
        * Every CTF map is able to support each and every single of the objective-based team gametypes, namely **CTF, Overload, CTF Elimination** and **Double Domination**.
        * CTF maps with a well-marked, contested central area can support **One Flag CTF** and **Harvester**.
...
You can download the current repo via commandline with *git clone https://github.com/NeonKnightOA/oacmpvol2* or by going to the URL and click *Clone/Download*. Then unzip into the OA directory with routes enabled.
...
* [egyptsoc pack (OACMP-adapted), nicked from Nexuiz](https://www.dropbox.com/s/aygrxhyqg909an8/oacmp2-egyptsoc-nexuiz.pk3?dl=0)
* [evillair packs (OACMP-adapted), nicked from Nexuiz](https://www.dropbox.com/s/bs3m90vvpduf3w8/oacmp2-evillair-nexuiz.pk3?dl=0)
* [Nexuiz models](https://www.dropbox.com/s/rjyys4fc1x2rfnq/nexuiz-mapmodels.pk3?dl=0)